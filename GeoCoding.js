...
// erreur à la connection
      if (err) res.send({"errorMessage" : err});

// passage de la requête
      db.collection("adresse").find({"num": num, "voie": voie, "commune": commune, "cp": cp, "pays": pays},
...
// erreur à la connection
      if (err) res.send({"errorMessage" : err});
// passage de la requête
      db.collection("adresse").findOne({location: {$geoNear: {$geometry: {"type": "Point" , "coordinates": [lat,lon]}}}}, function(err, adresse) {
// erreur lors du find
        if (err) res.send({"errorMessage" : err});
// fermeture de la base
        db.close();
...
// service non reconnu
    ...
    res.send({"errorMessage" : "Malformed request"});
  }
